...
int main(int argc, char* argv[])
{
    ...
    if(normal_on)
    {
        normal_ins = new Image(width,height);
        normal_ins->SetAllPixels(Vec3f(0,0,0));
    }
    Image* depth_ins;
    if(depth_on)
    {
        depth_ins = new Image(width,height);
        ...
        for (int j=0;j<height;j++)
        {
            ...
            if(flag=SP.getGroup()->intersect(ray,hit,tmin))
            {
                ...
                for (int k=0; k<num_light;k++)
                {
                    ...
                    Vec3f light_color;
                    float d;
                    SP.getLight(k)->getIllumination(hit.getIntersectionPoint(),light_dir,light_color);
                    if(shade_back && hit.getNormal().Dot3(ray.getDirection())>0)
                        d = light_dir.Dot3(Vec3f(0,0,0)-hit.getNormal());
                    else
                        d = light_dir.Dot3(hit.getNormal());
                    d = max(float(0),d);
                    tot_light += Vec3f(hit_color[0]*light_color[0]*d,hit_color[1]*light_color[1]*d,hit_color[2]*light_color[2]*d);
                }

...
bool Grid::intersect(const Ray& r, Hit& h, float tmin)
{
	...
	{
		while (mi.getindex(0) >= 0 && mi.getindex(0) < x && mi.getindex(1) >= 0 && mi.getindex(1) < y && mi.getindex(2) >= 0 && mi.getindex(2) < z)
		{
			int num = getNumObject(mi.getindex(0), mi.getindex(1), mi.getindex(2));
			if (num)
			{
				PhongMaterial* m;
				switch (num) {

...
Vec3f RayTracer::RayCast(Ray& ray, float tmin, int bounces, float w, Hit& hit) const
{
    ...
    if (SP->getGroup()->intersect(ray, hit, tmin))
    {
        ...
        if (bounces < bounces_num && w > weight && dec > 0)
        {
            ...
            Vec3f reflect_color = RayCast(reflect_ray, 0.001, bounces + 1, w * dec, reflect_hit);
        ...
        if (bounces < bounces_num && w > weight && dec_tra > 0 && par_or_not)
        {
            ...
            Vec3f trans_color = RayCast(trans_ray, 0.001, bounces + 1, w * dec_tra, trans_hit);
...
Vec3f RayTracer::RayCastFast(Ray& ray, float tmin, int bounces, float w, Hit& hit) const
{
    if (grid->intersect(ray, hit, tmin))
    {
        ...
        for (int k = 0; k < num_light; k++)
        {
            ...
            {
                RayTracingStats::IncrementNumShadowRays();
                if (!grid->ShodowJudge(shadow_ray, shadow_hit, 0.001) || shadow_hit.getT() > light_dis)
                {
                    shade_color = material->Shade(ray, hit, light_dir, light_color, light_dis);
                    tot_light += shade_color;
                }
        ...
        if (bounces < bounces_num && w > weight && dec > 0)
        {
            ...
            Ray reflect_ray = Ray(hit_point, reflect_dir);
            Hit reflect_hit = Hit(10000, NULL, Vec3f(0, 0, 0));
            Vec3f reflect_color = RayCastFast(reflect_ray, 0.001, bounces + 1, w * dec, reflect_hit);            
            tot_light += reflect_color * mat_ref_color;
            RayTree::AddReflectedSegment(reflect_ray, tmin, reflect_hit.getT());
        }
        ...
        if (bounces < bounces_num && w > weight && dec_tra > 0 && par_or_not)
        {
            Ray trans_ray = Ray(hit_point, transmitted_dir);
            Hit trans_hit = Hit(10000, NULL, Vec3f(0, 0, 0));
            Vec3f trans_color = RayCastFast(trans_ray, 0.001, bounces + 1, w * dec_tra, trans_hit);
            tot_light += trans_color * mat_tra_color;
            RayTree::AddTransmittedSegment(trans_ray, tmin, trans_hit.getT());
        }
        return tot_light;
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float w, Hit& hit) const
{
    ...
    if (visualize_grid)
    {
        ...
        if (grid->intersect(ray, hit, tmin))
        {
            ...
            for (int k = 0; k < SP->getNumLights(); k++)
            {
                ...
                color += hit.getMaterial()->Shade(ray, hit, l, lightColor, dis);
            }
            return color;
        }
        else
    ...
    if (fastrender)
        return RayCastFast(ray, tmin, bounces, w, hit);
    else
        return RayCast(ray, tmin, bounces, w, hit);
}

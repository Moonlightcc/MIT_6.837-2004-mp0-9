...
void Plane::insertIntoGrid(Grid* g, Matrix* m)
{
    if (m)
    {
        Vec3f* p = new Vec3f(point);
        Vec3f* n = new Vec3f(normal);
        m->Transform(*p);
        m->TransformDirection(*n);
        float d = p->Dot3(*n);
        Plane* newPlane = new Plane(*n, d, get_material());
        g->setInfinitePri(newPlane);
    }
    else
        g->setInfinitePri(this);
}
...
void Triangle::insertIntoGrid(Grid* g, Matrix* m)
{
    ...
    for (int i = start_index_i; i <= end_index_i; i++)
        for (int j = start_index_j; j <= end_index_j; j++)
            ...
            {
                if (m)
                    g->set(i, j, k, new Transform(*m, this));
                else
                    g->set(i, j, k, this);
            }
}
...
void Object3DVector::ClearDetect()
{
    for (int i = 0; i < num_objects; i++)
        objects[i]->InitDetect();
}